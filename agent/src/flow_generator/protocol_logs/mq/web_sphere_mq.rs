...
impl WebSphereMqInfo {
    ...
    pub fn merge_parsed_info(&mut self, mut parser: WebSphereMqParser) {
        ...
        if self.msg_type == LogMessageType::Response {
            self.span_id = std::mem::take(&mut parser.msg_id);
        } else {
            self.span_id = std::mem::take(&mut parser.mesg_id);
        }

        let trace_id = if !parser.ntfctn_id.is_empty() {
            std::mem::take(&mut parser.ntfctn_id)
        } else if !parser.orgnl_msg_id.is_empty() {
            std::mem::take(&mut parser.orgnl_msg_id)
        } else {
            std::mem::take(&mut parser.msg_id)
        };
        self.trace_ids
            .merge_field(BASE_FIELD_PRIORITY, self.span_id.clone());
        self.trace_ids.merge_field(BASE_FIELD_PRIORITY, trace_id);
...
impl L7ProtocolParserInterface for WebSphereMqLog {
    ...
    fn parse_payload(&mut self, payload: &[u8], param: &ParseParam) -> Result<L7ParseResult> {
        ...
        let custom_policies = config
            .l7_log_dynamic
            .get_custom_field_policies(L7Protocol::WebSphereMq.into(), param);
